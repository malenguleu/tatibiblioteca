...
exports.login = async (req, res) => {
    ...
    try {
        ...
        if (!user) return res.status(400).json({ error: 'Usuário não encontrado' }); // Retorna erro se o usuário não exister
        ...
        res.josn({ token }); // Responde com o token JWT
    } catch (error) {
        res.status(500).json({ error: 'Erro ao fazer login' }); // Responde com o erro ao fazer login
    }
};